...
const local = require("./localStrategy"); //로컬 서버로 로그인할 때
//const kakao = require('./kakaoStrategy'); //카카오 서버로 로그인할 때
...
module.exports = () => {
  ...
  //? req.login(user, ...)가 실행되면, seralizeUser가 실행된다.
  //? 즉, 로그인 과정을 할때만 실행
  passport.serializeUser((user, done) => {
    // req.login(user, ...)의 user가 일로 와서 값을 이용할수 있는 것이다.
    done(null, user.status); //req.session에 user.id만 저장
