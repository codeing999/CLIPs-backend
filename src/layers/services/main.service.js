      ...
      if (!category) {
        for (let k = 0; k < newArr.length; k++) {
          ...
          if (!responseImageData.length) {
            ...
            return {
              message: `${location}의 ${newArr[k]}이/가 ${responseImageData.length} 개 있습니다.`, //${newArr[k]}
        ...
        for (let k = 0; k < newArr.length; k++) {
          ...
          if (!responseImageData.length) {
            throw new Error(
              `${location}의 ${category}이/가 존재하지 않습니다. 재검색해보세요`
            );
          } else {
            return {
              message: `${location}의 ${category}이/가 ${responseImageData.length} 개 있습니다.`, //${newArr[k]}
              data: responseImageData,
            };
          }
        }
  ...
  crawlData = async (placeUrl) => {
    try {
      ...
      if (rawArrDateUrl.includes("더보기")) {
        ...
        let crawlTimeData1 = rawArrDateUrl.slice(index, index2);
        return { crawlingUrllist, crawlTimeData1 };
      } else {
        let crawlTimeData2 = rawArrDateUrl[0];
        return { crawlingUrllist, crawlTimeData2 };
      }
    } catch (err) {
      console.log(err);
      return { message: err.message };
